...
public class XMLMapperBuilder extends BaseBuilder {
  ...
  // 解析一个mapper.xml文件
  public void parse() {
    // resource是 xxxMapper.xml
    if (!configuration.isResourceLoaded(resource)) {
      configurationElement(parser.evalNode("/mapper"));
      configuration.addLoadedResource(resource);// 过了这一步之后，资源已加载
  ...
  private void configurationElement(XNode context) {
    try {
      ...
      /*
      MapperBuilderAssistant里面有这几个属性：
      String currentNamespace;
      String resource;
      Cache currentCache;
      boolean unresolvedCacheRef; // issue #676
       */
      builderAssistant.setCurrentNamespace(namespace);
      // cache-ref 其他命名空间缓存配置的引用
      cacheRefElement(context.evalNode("cache-ref"));
      // mapper.xml中配置是否使用二级缓存的标签 <cache/>
  ...
  private void buildStatementFromContext(List<XNode> list, String requiredDatabaseId) {
    for (XNode context : list) {
        // 又new 了一个 XMLStatementBuilder 来解析每一个增删改标签。
        // 也是在循环中，每一个 <select/> <update/> <delete/> <insert/> 标签都对应了一个 XMLStatementBuider 。
  ...
  private void cacheElement(XNode context) throws Exception {
    if (context != null) {
        // 默认使用 org.apache.ibatis.cache.impl.PerpetualCache 作二级缓存 它实际上是一个HashMap。
        // 后面使用装饰者模式对它进行层层的装饰。
      String type = context.getStringAttribute("type", "PERPETUAL");// 可以自定义缓存
      Class<? extends Cache> typeClass = typeAliasRegistry.resolveAlias(type);
      String eviction = context.getStringAttribute("eviction", "LRU");// 缓存策略，默认LRU
      ...
      // 在这个方法中对原始的 PerpetualCache 进行装饰
